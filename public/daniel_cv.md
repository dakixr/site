...
- Built a decision-tree DSL so non-engineers could update client logic, plus a VS Code extension with live diagrams, code actions and CLI tools.
- Established unit and snapshot testing to prevent regressions across framework updates.

#### Bank of Luxembourg Investments (BLI)
- Saved 100+ hours per quarter through automation; built internal tools with Django, HTMX and REST APIs.
- Replaced slow file scans with an indexed, cached query service for client inquiries, with monitoring, alerting and operational runbooks.

#### Deloitte (first mission)
- Built Stelar, a Python library for portfolio risk calculations, and a configuration-driven ETL toolkit using YAML and Excel specifications.
